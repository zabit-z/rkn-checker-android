...
## 💻 Сборка и запуск на Mac

### Способ 1: Прямой запуск из терминала (Native macOS App)
Приложение полностью совместимо с macOS и запускается в отдельном окне на рабочем столе одной командой:
```bash
cd ios
swift run
```

### Способ 2: В среде Xcode (iPad, iPhone или Mac)
Откройте проект:
```bash
open ios/RKNChecker.xcodeproj
```
В Xcode доступны две схемы запуска:
1. **`RKNCheckerMac`**: нативное приложение для **My Mac** (запуск по **Cmd + R**).
2. **`RKNChecker`**: мобильное приложение для симулятора **iPad Pro / iPad Air** или **iPhone** (запуск по **Cmd + R**).

### Способ 3: Запуск тестов из командной строки
```bash
cd ios
swift test
```

### Способ 4: Регенерация проекта через XcodeGen
Если вы изменили `project.yml` или структуру файлов:
```bash
cd ios
xcodegen generate
```
